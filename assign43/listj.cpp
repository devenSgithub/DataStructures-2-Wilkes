  ...
  int list::Josephus(int passes){
    ...
    if(head == NULL){
      ...
      while(head->next != head){
        ...
        //store the num to be deleted
        tempnum = current->item;
        //move to next node or segfault
        current = current->next;
        //remove item that was out
        delcheck = deleteElement(tempnum);
        //print the item that was deleted
        if(delcheck){
          cout<<"Item number "<< tempnum << " was deleted" <<endl;
        }else{
          //<just a check>
      ...
      //return the item that won the game
      //tempnum = current->item;
      return current->item;
